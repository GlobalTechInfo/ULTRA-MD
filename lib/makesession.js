...
async function processTxtAndSaveCredentials(txt) {
  ...
  // Validate the megaCode (it should be 8-9 characters long, which is typical for MEGA file hashes)
  if (megaCode.length < 8 || megaCode.length > 9) {
    console.error('Invalid megaCode length:', megaCode);
    return; // Exit if the code length is invalid
  }
